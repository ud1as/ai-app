...
class BotHandler:
    ...
    def setup_routes(self):
        ...
        self.router.post("/{bot_id}/configure", response_model=MessageResponse)(
            self.configure_bot
        )
    ...
    async def get_bot(
        self, bot_id: str, tenant_id: str = Depends(get_current_user_tenant)
    ):
        try:
    ...
    async def chat(
        ...
        try:
            ...
            response = self.chat_service.chat(bot_id=bot_id, message=request.query)

            return ChatResponse(
                answer=response["response"], relevant_context=response["context"]
            )
        except Exception as e:
            raise HTTPException(status_code=400, detail=str(e))
