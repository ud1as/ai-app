...
from data import DatasetResponse, FilePreviewResponse, FileProcessResponse
...
class KnowledgeHandler:
    ...
    def setup_routes(self):
        self.router.post("/preview", response_model=FilePreviewResponse)(
            self.preview_file
        )
        self.router.post("/process", response_model=FileProcessResponse)(
            self.process_file
        )
        self.router.get("/datasets", response_model=List[DatasetResponse])(
            self.get_datasets
        )
    ...
    async def preview_file(
        ...
        try:
            ...
            success, chunks, error = self.file_service.get_chunk_preview(
                content, tenant_id, filename=file.filename
            )
            if not success:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error previewing file: {str(e)}"
            )
    ...
    async def process_file(
        ...
        try:
            ...
            success, result = self.file_service.process_file(
                file_content=file_content, filename=file.filename, tenant_id=tenant_id
            )
    ...
    async def get_datasets(self, tenant_id: str = Depends(get_current_user_tenant)):
        try:
